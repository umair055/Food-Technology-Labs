...
export default function CrazyFoodChatbot() {
    ...
    const [messages, setMessages] = useState([
        { text: "Hello! I'm your crazy food assistant. What deliciousness can I help you with today?", isUser: false }
    ])
    ...
    const handleSubmit = (e) => {
        ...
        if (input.trim()) {
            ...
            setTimeout(() => {
                setMessages(prev => [...prev, { text: `Yum! That sounds delicious. Here's a random food fact: Did you know that the world's largest pizza was 122 feet in diameter? 🍕`, isUser: false }])
            }, 1000)
        }
    }
            ...
            {isOpen && (
                <div className="fixed bottom-0 right-0 m-4 bg-white border rounded-lg shadow-lg w-80">
                    <div className="flex justify-between items-center bg-gray-800 text-white p-2 rounded-t-lg">
                        ...
                        <button onClick={toggleChat} className="text-white hover:text-orange-200 animate-pulse">
                            <X className="h-6 w-6" />
                        </button>
                    </div>
